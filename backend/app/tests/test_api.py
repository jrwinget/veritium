...
class TestDocumentEndpoints:
    ...
    def test_upload_from_invalid_url(self, client):
        response = client.post("/api/v1/documents/upload", data={"url": "not-a-url"})
        assert response.status_code == 400

    def test_upload_from_invalid_doi(self, client):
        response = client.post("/api/v1/documents/upload", data={"doi": "invalid-doi"})
        assert response.status_code == 400
...
class TestAssessmentEndpoints:
    def test_create_assessment_invalid_document(self, client):
        response = client.post(
            "/api/v1/assessments/",
            json={"document_id": 99999, "user_claim": "Test claim"},
        )
    ...
    def test_submit_feedback_invalid_assessment(self, client):
        response = client.post(
            ...
            json={"feedback_score": 1, "feedback_comment": "Test comment"},
        )
        assert response.status_code == 404
