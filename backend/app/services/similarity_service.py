...
class SimilarityService:
    ...
    async def calculate_similarity(
        self, user_claim: str, document_claims: List[str], document_content: str
    ) -> Dict[str, Any]:
        # Clean and prepare texts
        user_claim_clean = self._preprocess_text(user_claim)
        document_claims_clean = [
            self._preprocess_text(claim) for claim in document_claims
        ]
        ...
        return {
            ...
            "best_matching_claim": document_claims[best_match_idx]
            if best_match_idx >= 0
            else "",
    ...
    async def _calculate_semantic_similarity(
        self, user_claim: str, document_claims: List[str]
    ) -> List[float]:
        self._load_model()
    ...
    async def _calculate_lexical_similarity(
        ...
        try:
            ...
            return [
                self._word_overlap_similarity(user_claim, claim)
    ...
    async def _find_evidence_snippets(
        self, user_claim: str, document_content: str
    ) -> List[Dict[str, Any]]:
    ...
    async def calculate_entailment_score(
        self, user_claim: str, evidence_text: str
    ) -> Dict[str, Any]:
        ...
        contradiction_count = sum(
            1 for indicator in contradiction_indicators if indicator in evidence_lower
        )
        support_count = sum(
            1 for indicator in support_indicators if indicator in evidence_lower
        )
