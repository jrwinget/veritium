...
class ClaimExtractor:
    ...
    async def extract_claims(self, text: str) -> List[str]:
        ...
        unique_claims = []
        for claim in claims:
            if not any(self._are_similar_claims(claim, existing) for existing in unique_claims):
    ...
    def _extract_sentences_with_claims(self, text: str) -> List[str]:
        ...
        claim_indicators = [
            'significant', 'significantly', 'correlation', 'associated', 'effect',
            'increased', 'decreased', 'higher', 'lower', 'improvement', 'reduction',
            'evidence', 'support', 'indicate', 'suggest', 'demonstrate', 'show',
            'result', 'finding', 'outcome', 'impact', 'influence', 'relationship'
        ]
    ...
    def _clean_claim(self, claim: str) -> str:
        ...
        claim = re.sub(r'^(?:that|which|who|where|when)\s+', '', claim, flags=re.I)  # Remove leading conjunctions
        return claim
    ...
    async def analyze_method_quality_indicators(self, text: str) -> Dict[str, Any]:
        ...
        sample_size_match = re.search(r'(?i)(?:n\s*=\s*|sample size.*?|participants.*?)(\d+)', text)
