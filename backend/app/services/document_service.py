...
class DocumentService:
    ...
    async def process_uploaded_file(self, file: UploadFile) -> Document:
        ...
        extracted_data = await self.text_extractor.extract_from_file(file_path, file_extension)
        
        # Extract claims
        claims = await self.claim_extractor.extract_claims(extracted_data["content"])
        
        # Calculate quality score
        quality_score = await self.quality_scorer.calculate_score(extracted_data["content"], claims)
    ...
    async def process_url(self, url: str) -> Document:
        try:
            ...
            if url.lower().endswith('.pdf') or 'application/pdf' in response.headers.get('content-type', ''):
                ...
                extracted_data = await self.text_extractor.extract_from_file(file_path, "pdf")
            else:
                # Extract from HTML
                extracted_data = await self.text_extractor.extract_from_html(response.text, url)
                file_path = None
    ...
    async def process_doi(self, doi: str) -> Document:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                full_text = ""
                try:
                    pdf_response = requests.get(doi_url, timeout=30, allow_redirects=True)
                    if 'application/pdf' in pdf_response.headers.get('content-type', ''):
                        ...
                        # Try HTML extraction
                        extracted_data = await self.text_extractor.extract_from_html(pdf_response.text, doi_url)
                        full_text = extracted_data["content"]
                        file_path = None
                except:
                ...
                # Calculate quality score
                quality_score = await self.quality_scorer.calculate_score(full_text, claims)
