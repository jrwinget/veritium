...
class ExplanationService:
    ...
    async def generate_explanation(
        ...
        evidence_explanation = self._explain_evidence(similarity_result['evidence_snippets'])
    ...
    def _explain_limitations(self, confidence_score: float, document: Document) -> str:
        ...
        if confidence_score < 0.5:
            limitations.append("The assessment has low confidence due to limited evidence.")
        ...
        if document.method_quality_score < 0.4:
            limitations.append("The study's methodological limitations affect result reliability.")
    ...
    def _generate_methodology_section(self, quality_assessment: Dict[str, Any]) -> str:
        ...
        if sample_details.get('max_sample_size'):
            ...
            if size >= 1000:
                sections.append(f"Large sample size (n={size}) enhances result reliability.")
            elif size >= 100:
                sections.append(f"Adequate sample size (n={size}) supports findings.")
            else:
                sections.append(f"Small sample size (n={size}) limits generalizability.")
        ...
        return " ".join(sections) if sections else "Limited methodological information available."
    ...
    def _assess_overclaiming(
        ...
        absolute_terms = ['always', 'never', 'all', 'none', 'completely', 'totally', 'definitely', 'certainly']
        has_absolute_language = any(term in user_claim.lower() for term in absolute_terms)
        ...
        if has_absolute_language and similarity_score < 0.7:
            ...
            return "The evidence provides limited support for strong claims in this area."
        elif similarity_score < 0.4:
            return "Your specific claim differs substantially from the study's findings."
        else:
