...
class QualityScorer:
    def __init__(self):
        self.quality_criteria = {
            ...
            'methodology': {
                'weight': 0.25,
                'indicators': [
                    'randomized', 'random assignment', 'control group', 'placebo',
                    'double blind', 'single blind', 'systematic', 'meta-analysis'
                ]
            },
            'statistical_rigor': {
                'weight': 0.20,
                'indicators': [
                    'confidence interval', 'significance test', 'p-value',
                    'statistical power', 'effect size', 'regression'
                ]
            },
            'peer_review': {
                'weight': 0.15,
                'indicators': [
                    'journal', 'published', 'peer review', 'doi'
                ]
            },
            'reproducibility': {
                'weight': 0.10,
                'indicators': [
                    'data availability', 'code availability', 'replication',
                    'supplementary material', 'methodology section'
                ]
            },
            'transparency': {
                'weight': 0.15,
                'indicators': [
                    'limitation', 'conflict of interest', 'funding',
    ...
    async def calculate_score(self, text: str, claims: List[str]) -> float:
        ...
        total_score = 0.0
        for criterion, weight in [(k, v['weight']) for k, v in self.quality_criteria.items()]:
    ...
    def _assess_methodology(self, text: str) -> float:
        ...
        high_quality_types = [
            ...
            'meta-analysis', 'longitudinal study', 'cohort study'
        ]
    ...
    def _assess_reproducibility(self, text: str) -> float:
        ...
        reproducibility_elements = [
            'github', 'repository', 'supplementary', 'appendix',
            'raw data', 'analysis code', 'protocol'
        ]
    ...
    def _assess_transparency(self, text: str) -> float:
        ...
        transparency_elements = [
            ...
            'consent', 'ethics committee', 'declaration'
        ]
    ...
    async def get_detailed_assessment(self, text: str, claims: List[str]) -> Dict[str, Any]:
