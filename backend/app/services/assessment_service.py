...
class AssessmentService:
    ...
    async def create_assessment(
        ...
        entailment_result = await self.similarity_service.calculate_entailment_score(
            user_claim=user_claim,
            evidence_text=document.content or ""
        )
    ...
    async def get_assessment_by_share_id(self, share_id: str) -> Optional[Assessment]:
        result = await self.db.execute(select(Assessment).where(Assessment.share_id == share_id))
        return result.scalar_one_or_none()
    ...
    def _calculate_evidence_strength(
        ...
        evidence_bonus = min(0.2, num_evidence * 0.05)  # Up to 0.2 bonus for multiple evidence
    ...
    def _calculate_confidence_score(
        ...
        score_variance = self._calculate_score_variance([
            similarity_score, entailment_score, method_quality_score, evidence_strength_score
        ])
